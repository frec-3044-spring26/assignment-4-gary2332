...
ice_data <- read_excel("lake_ice_off_data.xlsx", sheet = "data")
...
**Answer 2:**
Ice-off should be happening earlier over time as temperatures warm, so I expect a negative slope (ice_off_doy decreases as year increases).
...
**Answer 3:** 
The slope is negative. This matches my expectation from Question 2 because it shows ice-off is happening earlier as years increase.
...
**Answer 4:**
The R2 is 1.1%. This is very low, so year and ice-off day are not strongly related. Year only explains about 1% of the variation in ice-off dates.
...
**Answer 6:**
Yes, the residuals meet the assumptions pretty well. The histogram is roughly bell-shaped and centered around zero, so they're approximately normally distributed with no major bias.
```{r}
...
**Answer 7:**
The p-value for the slope is 0.0581.
...
**Answer 8:**
It would take about 226 years for a 1 week change. I don't think this is very meaningful because 226 years is a really long time, and many other factors would change over that period.
...
**Answer 9:** 
I'm not very comfortable using this model for predictions. The R2 is really low, meaning year explains almost nothing about ice-off dates. Also, the p-value is 0.058, which is above the usual 0.05 cutoff, so we can't even say with confidence that the slope is different from zero.
...
ci_lower <- pred_2040 + lower_resid
ci_upper <- pred_2040 + upper_resid
ci_lower
ci_upper
...
ggplot() +
  geom_point(data = sunapee_early, aes(x = year, y = ice_off_doy), color = "blue") +
  geom_smooth(data = sunapee_early, aes(x = year, y = ice_off_doy), method = "lm", color = "blue", se = FALSE) +
  geom_point(data = sunapee_late, aes(x = year, y = ice_off_doy), color = "red") +
  geom_smooth(data = sunapee_late, aes(x = year, y = ice_off_doy), method = "lm", color = "red", se = FALSE) +
  labs(title = "Lake Sunapee Ice-Off: Before and After 1970",
       x = "Year", y = "Ice-off day")
```
The early slope is -0.0018 almost flat and the recent slope is -0.21 which is much steeper. So yes, ice-off is changing faster in recent years.
